...
— a string that looks like zero

```javascript
describeValue(NaN)
```

— what does `typeof NaN` give?

```javascript
describeValue(null)
```

— remember the strange thing we saw in class

### Hint:

You do not need a long if chain to find the type. `typeof` gives it to you. And a value can be put straight inside an `if` to test truthy or falsy.
...
Two case lines can share one result if you do not put `break` between them. And a string method can fix the letter case before the switch starts.
...
`.length`, `.includes()`, and `.toLowerCase()`.
...
Build the fare step by step in a `let` variable, then apply the night charge at the very end.
...
Use a template string for the sentence. Write it as:
